...
--TODO функция занятости. После спавна всех светофоров для каждого ссветофора создается (для каждого некст сигнала) список ноудов, которым принадлежит блок участок этого светофора (до следующиего (берется из списка некст сигналов (если сигнал на другом треке, то тот трек тоже надо учитывать (чтобы узнать, в каком направлении надо учитывать трек, запустить поиск этих двух сигналов друг относительно друга)), либо некст сигнал линк), либо до конца трека). И как только состав на нужном треке и на нужном ноуде, то занятость активируется
--[[
Если сигнал указывает или может указывать на репитер, то будет двойной отрезок, который потом объединится
]]

--TODO телепорт на трек по айди и по ноуду

local RawOccupationSections = {}
local function GenerateRawOccupationSections()
end

local function GenerateNormalOccupationSection(startSignal)
end
