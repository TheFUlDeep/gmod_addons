...
function ENT:Animate(clientProp, value, min, max, speed, damping, stickyness)
    ...
    if damping == false then
        ...
        local max_speed = 1.5*delta / self.DeltaTime --+ math.random(0, 3)			-- эмуляция поломанности при погасании. При отрицательном значении мигает всегда
        ...
        self.Anims[id].V = self.Anims[id].V + dX2dT * 10 --self.DeltaTime			-- мнговенное погасание
...
function ENT:Think()
    ...
    if not self.ModelsCreated then
            ...
            if v ~= "M" then
                for i = 1,#v do
                    ...
                    if tonumber(self.Sig[ID2]) and self.Signals[ID2].RealState ~= (tonumber(self.Sig[ID2]) > 0) then
                        ...
                        self.Signals[ID2].Stop = CurTime() + 0.4						-- погасание линзы при закрытии/открытии
                    end
                    if self.Signals[ID2].Stop and CurTime()-self.Signals[ID2].Stop > 0 then
                        self.Signals[ID2].Stop = nil
                    end
					local interval = 0.7		-- моя переменная, которой я задаю интервал мигания
                    local State = self:Animate(ID.."/"..i,  ((tonumber(self.Sig[ID2]) == 1 or (tonumber(self.Sig[ID2]) == 2 and (RealTime() % interval > interval/2))) and not self.Signals[ID2].Stop) and 1 or 0,  0.01,1, 128)		-- сам светофор
					--State3 = State
                ...
                if Metrostroi.RoutePointer[self.Num[1]] and IsValid(self.Models[1][self.RouteNumber]) then 
					if self:GetNW2Bool("RED") then self.Num = "" end
					self.Models[1][self.RouteNumber]:SetSkin(Metrostroi.RoutePointer[self.Num[1]])
					--print(State3)
				end
            end
        ...
        for k,v in pairs(self.RouteNumbers) do
            ...
            local State1 = self:Animate("rou1"..k,self.Num:find(v[1]) and 1 or 0,   0.005,1, 256)		-- маршрутный указатель
            local State2
            --if v[3] then
            if v[2] then State2 = self:Animate("rou2"..k,self.Num:find(v[2])and 1 or 0,     0.005,1, 256) end			-- маршрутный указатель
            if not IsValid(self.Models[3]["rou1"..k]) and State1 > 0 then
				if not v.pos or not tonumber(v[1]) then continue end					-- фикс ошибки на неомалине
            ...
            if IsValid(self.Models[3]["rou1"..k]) then
				if self:GetNW2Bool("RED") then State1 = self:Animate("rou1"..k,0,   0.005,1, 256) end
                if State1 > 0 then
            ...
            if IsValid(self.Models[3]["rou2"..k]) then
				if self:GetNW2Bool("RED") then State2 = self:Animate("rou2"..k,0,     0.005,1, 256) end
                if State2 > 0 then
