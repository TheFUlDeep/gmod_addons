...
if not GetConVar("show_rcs_names") then
CreateClientConVar("show_rcs_names","1",true,false,"", 0, 1)
end
...
hook.Add( "PopulateToolMenu", "MetrostroiCustomPanel", function()
	spawnmenu.AddToolMenuOption( "Utilities", "Metrostroi", "metrostroi_client_panel2", "Клиент2", "", "", function(panel)
		...
		panel:CheckBox("Отображать имена рельсовых цепей","show_rcs_names")
		panel:CheckBox("Кастомные пассажиры","metrostroi_custom_passengers")
		panel:CheckBox("Вкл./выкл. многопоточный рендеринг при загрузке","multi_thread_rendering_enabled")
		panel:CheckBox("Не трогать многопоточный рендеринг при загрузке","dont_touch_multi_thread_rendering")
